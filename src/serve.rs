...
fn get_metrics(rpc: Arc<Client>) -> Result<(), Error> {
		...
		Ok(networkinfo) => {
			match rpc.get_blockchain_info() {
				Ok(blockchaininfo) => {
					...
					if let Ok(latest_blockstats) = rpc
						.get_block_stats2(HashOrHeight::Hash(blockchaininfo.best_block_hash), None)
					{
	...
	match rpc.estimate_smart_fee(2, None) {
		Ok(smartfee) => {
	...
	match rpc.estimate_smart_fee(3, None) {
		Ok(smartfee) => {
			if let Some(fee_rate) = smartfee.fee_rate {
	...
	match rpc.estimate_smart_fee(5, None) {
		Ok(smartfee) => {
			if let Some(fee_rate) = smartfee.fee_rate {
	...
	match rpc.estimate_smart_fee(20, None) {
		Ok(smartfee) => {
			if let Some(fee_rate) = smartfee.fee_rate {
	...
	match rpc.get_network_hash_ps(Some(120), None) {
		Ok(hashps) => {
	...
	match rpc.get_network_hash_ps(Some(0), None) {
		Ok(hashps) => {
	...
	match rpc.get_network_hash_ps(Some(1), None) {
		Ok(hashps) => {
	...
	match rpc.get_chain_tx_stats(None, None) {
		Ok(txstats) => BITCOIN_TXCOUNT.set(txstats.txcount as f64),
		Err(e) => return Err(e),
	}

	match rpc.get_chain_tips() {
		Ok(chaintips) => BITCOIN_NUM_CHAINTIPS.set(chaintips.len() as f64),
		Err(e) => return Err(e),
	}

	match rpc.get_memory_info() {
		Ok(meminfo) => {
	...
	match rpc.get_rpc_info() {
		Ok(rpcinfo) => BITCOIN_RPC_ACTIVE.set((rpcinfo.active_commands.len() - 1) as f64),
		Err(e) => return Err(e),
	}
